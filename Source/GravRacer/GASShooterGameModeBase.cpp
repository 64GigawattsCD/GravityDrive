...
AGASShooterGameModeBase::AGASShooterGameModeBase()
{
	...
	PawnClass = StaticLoadClass(UObject::StaticClass(), nullptr, TEXT("/Game/GASShooter/Characters/Hero/BP_HeroCharacter.BP_HeroCharacter_C"));
	if (!PawnClass)
	{
...
void AGASShooterGameModeBase::RespawnHero(AController* Controller)
{
	...
	if (Controller->IsPlayerController())
	{
		...
		AGravRacerPawn* Hero = GetWorld()->SpawnActor<AGravRacerPawn>(PawnClass, PlayerStart->GetActorLocation(), PlayerStart->GetActorRotation(), SpawnParameters);
	...
	else
	{
		...
		AGravRacerPawn* Hero = GetWorld()->SpawnActor<AGravRacerPawn>(PawnClass, EnemySpawnPoint->GetActorTransform(), SpawnParameters);
