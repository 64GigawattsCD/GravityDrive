...
{
	AGravRacerAIController* MyController = Cast<AGravRacerAIController>(InActor);
	AGravRacerPawn* MyBot = MyController ? Cast<AGravRacerPawn>(MyController->GetPawn()) : NULL;
				...
				if (Hit.GetActor() != NULL)
				{
					...
					else
					{
							...
							&& (MyController->PlayerState != NULL) && (HitChar->GetPlayerState() != NULL))
						{
							...
							AGSPlayerState* MyPlayerState = Cast<AGSPlayerState>(MyController->PlayerState);
