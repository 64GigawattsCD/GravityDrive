...
//class declare
class SGravRacerMenuWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SGravRacerMenuWidget)
	: _MenuHUD()
	{}
	...
	/** add item to menu */
	static FGravRacerMenuItem* AddMenuItem(MenuPtr &SubMenu, FText Text);

	/** add custom widget to menu (for example whole controls remapping window) */
	static FGravRacerMenuItem* AddCustomWidgetMenuItem(MenuPtr &SubMenu, TSharedPtr<SWidget> CustomWidget);
...
private:
	...
	TSharedPtr< TArray<class FGravRacerMenuItem> > PendingLeftMenu;
	...
	/** style for this menu */
	const struct FGravRacerMenuStyle *MenuStyle;
};
