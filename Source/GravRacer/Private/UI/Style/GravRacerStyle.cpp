...
#include "GravRacerStyle.h"
#include "SlateGameResources.h"

TSharedPtr< FSlateStyleSet > FGravRacerStyle::GravRacerStyleInstance = nullptr;

void FGravRacerStyle::Initialize()
{
	if ( !GravRacerStyleInstance.IsValid() )
	{
		GravRacerStyleInstance = Create();
		FSlateStyleRegistry::RegisterSlateStyle( *GravRacerStyleInstance );
	}
}

void FGravRacerStyle::Shutdown()
{
	FSlateStyleRegistry::UnRegisterSlateStyle( *GravRacerStyleInstance );
	ensure( GravRacerStyleInstance.IsUnique() );
	GravRacerStyleInstance.Reset();
}

FName FGravRacerStyle::GetStyleSetName()
{
	static FName StyleSetName(TEXT("GravRacerStyle"));
	return StyleSetName;
}
...
TSharedRef< FSlateStyleSet > FGravRacerStyle::Create()
{
	TSharedRef<FSlateStyleSet> StyleRef = FSlateGameResources::New(FGravRacerStyle::GetStyleSetName(), "/Game/UI/Styles", "/Game/UI/Styles");
	...
	Style.Set("GravRacer.MenuTextStyle", FTextBlockStyle()
...
void FGravRacerStyle::ReloadTextures()
{
	FSlateApplication::Get().GetRenderer()->ReloadTextureResources();
}

const ISlateStyle& FGravRacerStyle::Get()
{
	return *GravRacerStyleInstance;
}
