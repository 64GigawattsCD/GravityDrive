...
#include "SGravRacerControlsSetup.h"
...
#define LOCTEXT_NAMESPACE "SGravRacerControlsSetup"

void SGravRacerControlsSetup::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			.OnGenerateRow(this, &SGravRacerControlsSetup::MakeListViewWidget)
			.OnSelectionChanged(this, &SGravRacerControlsSetup::KeyMapSelectionChanged)
			.OnMouseButtonDoubleClick(this,&SGravRacerControlsSetup::OnListItemDoubleClicked)
		]
	];

}

void SGravRacerControlsSetup::UpdateActionBindings(FName GroupName)
{
	if (!GEngine)
	{
		return;
...
void SGravRacerControlsSetup::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SGravRacerControlsSetup::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SGravRacerControlsSetup::KeyMapSelectionChanged(TSharedPtr<FActionBinding> InItem, ESelectInfo::Type SelectInfo)
{
...
void SGravRacerControlsSetup::OnListItemDoubleClicked(TSharedPtr<FActionBinding> InItem)
{
	SelectedItem = InItem;
...
void SGravRacerControlsSetup::MoveSelection(int32 MoveBy)
{
	int32 SelectedItemIndex = ActionBindings.IndexOfByKey(SelectedItem);
...
FReply SGravRacerControlsSetup::OnAnalogValueChanged(const FGeometry& MyGeometry, const FAnalogInputEvent& InAnalogInputEvent)
{
...
void SGravRacerControlsSetup::RemapKey(TSharedPtr<FActionBinding> Binding, const FKey Key, bool bForcedAxis, bool bForcedAction)
{
	if (GEngine)
	{
		if(!OwnerHUD.IsValid())
...
FReply SGravRacerControlsSetup::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent) 
{
	FReply Result = FReply::Unhandled();
...
TSharedRef<ITableRow> SGravRacerControlsSetup::MakeListViewWidget(TSharedPtr<FActionBinding> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	return
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text( this,  &SGravRacerControlsSetup::GetItemKeyBinding, Item)
					.TextStyle(FGravRacerStyle::Get(), "GravRacer.MenuTextStyle")
				]
			]
		];
...
void SGravRacerControlsSetup::PopulateFriendlyKeyNameMap()
{
...
TArray<FKey>* SGravRacerControlsSetup::AddFriendlyKeyNameRemap(const FKey& KeyID, const FString& InFriendlyName, const EConsoleForGamepadLabels::Type InControllerType)
{
...
FText SGravRacerControlsSetup::GetKeyText(FKey Key) const
{
...
FText SGravRacerControlsSetup::GetItemKeyBinding(TSharedPtr<FActionBinding> Item) const
{
...
BindingRestriction* SGravRacerControlsSetup::AddBindingRestriction(const FString& InControlName, bool InAxisAllowed, bool InActionAllowed)
{
...
bool SGravRacerControlsSetup::IsBindingAllowed(TSharedPtr<FActionBinding> Binding, const FKey Key, bool bForcedAxis, bool bForcedAction)
{
	for (int32 Bind = 0; Bind < BindingRestrictions.Num(); Bind++)
	{
		...
		{			
			const bool bKeyIsAxis = Key.IsAxis1D() || Key.IsAnalog();
